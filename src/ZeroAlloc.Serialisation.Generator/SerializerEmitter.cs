...
internal static class SerializerEmitter
{
    public static void Emit(SourceProductionContext ctx, SerializerModel model)
    {
        var serializerCall = model.FormatName switch
        {
            ...
            "SystemTextJson" =>
                $"using var _jw = new global::System.Text.Json.Utf8JsonWriter(writer);\n        global::System.Text.Json.JsonSerializer.Serialize(_jw, value, global::{model.StjContext!.ContextFullName}.Default.{model.StjContext.PropertyName});",
        ...
        // MemoryPack/MessagePack still route through APIs annotated with [RequiresUnreferencedCode] /
        // [RequiresDynamicCode] even though their own source generators emit the formatter; the
        // attributes below silence publish-time IL analysis on the bindings we own. STJ no longer
        // needs them — the JsonSerializerContext path is first-class AOT-safe.
        string suppressAttrs = model.FormatName == "SystemTextJson"
            ? ""
